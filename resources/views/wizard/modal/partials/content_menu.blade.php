<ul class="dashboard-tabs nav nav-pills nav-light-primary row row-paddingless m-0 p-0 flex-column flex-sm-row" role="tablist">
  {{-- begin::Sudoku --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" data="sudoku">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/sudoku.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Sudoku
      </span>
    </a>
  </li>
  {{-- end::Sudoku --}}
  {{-- begin::Akari --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" data="akari">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/akari.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Akari
      </span>
    </a>
  </li>
  {{-- end::Akari --}}
  {{-- begin::Domino --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" data="domino">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/domino.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Domino
      </span>
    </a>
  </li>
  {{-- end::Domino --}}
  {{-- begin::Fillomino --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" data="fillomino">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/fillomino.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Fillomino
      </span>
    </a>
  </li>
  {{-- end::Fillomino --}}
  {{-- begin::Futoshiki --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" data="futoshiki">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/futoshiki.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Futoshiki
      </span>
    </a>
  </li>
  {{-- end::Futoshiki --}}
  {{-- begin::Gokigen --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" href="#sudoku_options">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/gokigen.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Gokigen
      </span>
    </a>
  </li>
  {{-- end::Gokigen --}}
</ul>
<ul class="dashboard-tabs nav nav-pills nav-light-primary row row-paddingless m-0 p-0 flex-column flex-sm-row" role="tablist">
  {{-- begin::Sikaku --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" data="sikaku">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/sikaku.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Sikaku
      </span>
    </a>
  </li>
  {{-- end::Sikaku --}}
  {{-- begin::Kakuro --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" data="coming_soon">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/kakuro-muted.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Kakuro
      </span>
      <div class="row align-items-start w-100">
        <span class="label pulse pulse-info mr-1">
            <span class="pulse-ring"></span>
        </span>
        <span class="label label-info label-inline mr-1">Coming soon</span>
      </div>
    </a>
  </li>
  {{-- end::Kakuro --}}
  {{-- begin::Kendoku --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" data="coming_soon">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/kendoku-muted.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Kendoku
      </span>
      <div class="row align-items-start w-100">
        <span class="label pulse pulse-info mr-1">
            <span class="pulse-ring"></span>
        </span>
        <span class="label label-info label-inline mr-1">Coming soon</span>
      </div>
    </a>
  </li>
  {{-- end::Kendoku --}}
  {{-- begin::Ksudoku --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" data="coming_soon">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/ksudoku-muted.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Ksudoku
      </span>
      <div class="row align-items-start w-100">
        <span class="label pulse pulse-info mr-1">
            <span class="pulse-ring"></span>
        </span>
        <span class="label label-info label-inline mr-1">Coming soon</span>
      </div>
    </a>
  </li>
  {{-- end::Ksudoku --}}
  {{-- begin::Minesweeper --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" data="coming_soon">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/minesweeper-muted.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Mine Sweeper
      </span>
      <div class="row align-items-start w-100">
        <span class="label pulse pulse-info mr-1">
            <span class="pulse-ring"></span>
        </span>
        <span class="label label-info label-inline mr-1">Coming soon</span>
      </div>
    </a>
  </li>
  {{-- end::Minesweeper --}}
  {{-- begin::Murapeke --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" data="coming_soon">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/murapeke-muted.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Murapeke
      </span>
      <div class="row align-items-start w-100">
        <span class="label pulse pulse-info mr-1">
            <span class="pulse-ring"></span>
        </span>
        <span class="label label-info label-inline mr-1">Coming soon</span>
      </div>
    </a>
  </li>
  {{-- end::Murapeke --}}
</ul>
<ul class="dashboard-tabs nav nav-pills nav-light-primary row row-paddingless m-0 p-0 flex-column flex-sm-row" role="tablist">
  {{-- begin::Roundabouts --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" data="coming_soon">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/roundabout-muted.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Round Abouts
      </span>
      <div class="row align-items-start w-100">
        <span class="label pulse pulse-info mr-1">
            <span class="pulse-ring"></span>
        </span>
        <span class="label label-info label-inline mr-1">Coming soon</span>
      </div>
    </a>
  </li>
  {{-- end::Roundabouts --}}
  {{-- begin::Slitherlink --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" data="coming_soon">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/slitherlink-muted.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Slither Link
      </span>
      <div class="row align-items-start w-100">
        <span class="label pulse pulse-info mr-1">
            <span class="pulse-ring"></span>
        </span>
        <span class="label label-info label-inline mr-1">Coming soon</span>
      </div>
    </a>
  </li>
  {{-- end::Slither Link --}}
  {{-- begin::Tatami --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" data="coming_soon">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/tatami-muted.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Tatami
      </span>
      <div class="row align-items-start w-100">
        <span class="label pulse pulse-info mr-1">
            <span class="pulse-ring"></span>
        </span>
        <span class="label label-info label-inline mr-1">Coming soon</span>
      </div>
    </a>
  </li>
  {{-- end::Tatami --}}
  {{-- begin::Tents --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" data="coming_soon">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/tents-muted.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Tents
      </span>
      <div class="row align-items-start w-100">
        <span class="label pulse pulse-info mr-1">
            <span class="pulse-ring"></span>
        </span>
        <span class="label label-info label-inline mr-1">Coming soon</span>
      </div>
    </a>
  </li>
  {{-- end::Tents --}}
  {{-- begin::Jigsaw --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" data="coming_soon">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/jigsaw3-muted.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Jigsaw
      </span>
      <div class="row align-items-start w-100">
        <span class="label pulse pulse-info mr-1">
            <span class="pulse-ring"></span>
        </span>
        <span class="label label-info label-inline mr-1">Coming soon</span>
      </div>
    </a>
  </li>
  {{-- end::Jigsaw --}}
  {{-- begin::Coloring --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" data="coming_soon">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/coloring1-muted.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Coloring
      </span>
      <div class="row align-items-start w-100">
        <span class="label pulse pulse-info mr-1">
            <span class="pulse-ring"></span>
        </span>
        <span class="label label-info label-inline mr-1">Coming soon</span>
      </div>
    </a>
  </li>
  {{-- end::Coloring --}}
</ul>
<ul class="dashboard-tabs nav nav-pills nav-light-primary row row-paddingless m-0 p-0 flex-column flex-sm-row" role="tablist">
  {{-- begin::Activity --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" data="coming_soon">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/activity1-muted.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Activities
      </span>
      <div class="row align-items-start w-100">
        <span class="label pulse pulse-info mr-1">
            <span class="pulse-ring"></span>
        </span>
        <span class="label label-info label-inline mr-1">Coming soon</span>
      </div>
    </a>
  </li>
  {{-- end::Activity --}}
  {{-- begin::Journal --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
    <a class="nav-link border py-10 d-flex flex-grow-1 rounded flex-column align-items-center" data-toggle="pill" data="coming_soon">
      <span class="nav-icon py-2 w-auto">
          <img class="mw-50 w-100px" src="{{ asset('media/categories/journal1-muted.png') }}">
      </span>
      <span class="nav-text font-size-lg py-2 font-weight-bold text-center">
        Journal
      </span>
      <div class="row align-items-start w-100">
        <span class="label pulse pulse-info mr-1">
            <span class="pulse-ring"></span>
        </span>
        <span class="label label-info label-inline mr-1">Coming soon</span>
      </div>
    </a>
  </li>
  {{-- end::Journal --}}
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
  </li>
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
  </li>
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
  </li>
  <li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
  </li>
</ul>
